...
class TestInputs(BaseTest):
	...
	def test_simple_alert(self):
		self.wait.until(EC.element_to_be_clickable((By.XPATH, '//button[text()="Click for JS Alert"]'))).click()
		alert = self.driver.switch_to.alert
		alert.accept()
		assert self.get_result_text() == 'You successfuly clicked an alert'

	def test_confirm_alert_accept(self):
		self.wait.until(EC.element_to_be_clickable((By.XPATH, '//button[text()="Click for JS Confirm"]'))).click()
		alert = self.driver.switch_to.alert
		alert.accept()
	...
	def test_prompt_alert_with_text(self):
		...
		self.wait.until(EC.element_to_be_clickable((By.XPATH, '//button[text()="Click for JS Prompt"]'))).click()
		alert = self.driver.switch_to.alert
