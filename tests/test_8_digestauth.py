...
class TestDigestAuth:
	...
	def test_context_menu_positive(self):
		...
		assert WebDriverWait(self.driver, 5).until(EC.visibility_of_element_located((By.TAG_NAME, 'h3'))).text == 'Digest Auth'

	def test_context_menu_wrong_password(self):
		self.driver.get('http://admin:nottherightpassword@localhost:7080/digest_auth')
